...
// name.
func OpenPack(idxFile, idxPath string, algo *gitobj.Algo) (*Pack, error) {
	p := &Pack{
		IdxPath: idxPath,
		IdxFile: idxFile,
	...
	var err error
	if p.idxMap, err = mapReadOnly(idxFile); err != nil {
		return nil, err
	}
	...
	if p.dataMap, err = mapReadOnly(p.File); err != nil {
		p.Close()
		return nil, err
	}
