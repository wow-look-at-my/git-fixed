...
type Worktree struct {
	...
	IsMain bool
	// ID is the directory name under worktrees/, empty for the main one.
	ID string
}
...
// Worktrees lists the main worktree first, then the linked ones by name, which
// is the order git's get_worktrees() returns.
...
// does: the main worktree uses the plain name, a linked one is prefixed.
